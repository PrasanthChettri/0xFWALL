...
struct {
    ...
    __type(value, struct rule_value);
    __uint(map_flags, BPF_F_NO_PREALLOC);
} EGRESS_IPV6_RULE_MAP_ID SEC(".maps");
...
static __always_inline int check_ip6_blocklist(struct in6_addr daddr) {
    struct ipv6_rule_key dst_key = { .prefixlen = 128, .addr = daddr };
    struct rule_value *dst_rule = bpf_map_lookup_elem(&EGRESS_IPV6_RULE_MAP_ID, &dst_key);
    if (dst_rule && dst_rule->action == RULE_ACTION_DROP)
        return 1;
    return 0; 
}
